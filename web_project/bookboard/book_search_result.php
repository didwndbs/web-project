<!-- book_search_result.php: 제목, 저자, 출판사 검색 결과 -->
<!DOCTYPE html>
<head>
<meta charset="UTF-8">
...
<body id="top">
    ...
    <div class="wrapper row3">
      ...
      <table class="list-table">
        ...
        if($college && $major){
          if($catagory == "제목")
            $sql = mq("select * from book_board where (college like '$college%' and major = '$major') 
            and replace(title,' ','') like '%$search_result%' order by idx desc");
          elseif($catagory == "저자")
            $sql = mq("select * from book_board where (college like '$college%' and major = '$major') 
            and replace(bo_author,' ','') like '%$search_result%' order by idx desc");
          elseif($catagory == "출판사")
          $sql = mq("select * from book_board where (college like '$college%' and major = '$major') 
          and replace(bo_publisher,' ','') like '%$search_result%' order by idx desc");
        }
        elseif($college=="전공" || $college=="교양"){
          if($catagory == "제목")
          $sql = mq("select * from book_board where (category ='전공' or category = '교양') 
          and replace(title,' ','') like '%$search_result%' order by idx desc");
        elseif($catagory == "저자")
          $sql = mq("select * from book_board where (category ='전공' or category = '교양') 
          and replace(bo_author,' ','') like '%$search_result%' order by idx desc");
        elseif($catagory == "출판사")
        $sql = mq("select * from book_board where c(category ='전공' or category = '교양')  
        and replace(bo_publisher,' ','') like '%$search_result%' order by idx desc");
        }
        elseif($college){
          if($catagory == "제목")
          $sql = mq("select * from book_board where college = '$college' 
          and replace(title,' ','') like '%$search_result%' order by idx desc");
        elseif($catagory == "저자")
          $sql = mq("select * from book_board where college = '$college' 
          and replace(bo_author,' ','') like '%$search_result%' order by idx desc");
        ...
        elseif($major){
          if($catagory == "제목")
          $sql = mq("select * from book_board where major = '$major' 
          and replace(title,' ','') like '%$search_result%' order by idx desc");
        elseif($catagory == "저자")
          $sql = mq("select * from book_board where major = '$major' 
          and replace(bo_author,' ','') like '%$search_result%' order by idx desc");
